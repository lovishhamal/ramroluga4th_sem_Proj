...
function DashboardProduct() {
	...
	const onSubmit = async (e) => {
		...
		try {
			...
			if (formData === null && value === null && title === null && price === null && info === null) {
				return;
			}
			const res = axios.post('http://127.0.0.1:5000/product/upload', formData, {});
	...
	const listimages = (e) => {
		e.preventDefault();
		try {
			fetch('http://127.0.0.1:5000/product/getproduct' + email).then((res) => {
				res.json().then((data) => {
					setlist(data);
				});
			});
